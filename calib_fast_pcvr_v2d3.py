...
#版本v2d3
#针对v2d2，主要是更改评估信息
#运行代码：python calib_fast_pcvr_v2d3.py > ./result/hb_calibrationV2d3_result_fast_pcvr_flow1_1015.txt
#python calib_fast_pcvr_v2d3.py --test_day=1015 --train_days=1 --calib_method=1 > ./result/hb_calibrationV2d3_result_fast_pcvr_flow1_1015.txt
...
class ClassifierCalibrator:
    ...
    def _calculate_field_ece(self, df, prob_col, label_col, filed_col=None):
        """
        按filed列进行分箱，计算每个箱子的统计量及整体ECE
        
        参数:
            df: 包含预测概率、真实标签和ad列的DataFrame
            prob_col: 预测概率列名
            label_col: 真实标签列名
            filed_col: filed
        返回:
            tuple: (每个ad箱子的统计量DataFrame, 整体ECE值)
        """
        # 复制数据避免修改原DataFrame
        df = df.copy()
        
        # 核心：按ad列分组（每个ad作为一个独立的箱子），计算每个箱子的统计量
        bin_stats = df.groupby(filed_col).agg({
            prob_col: ['mean'],         # 每个ad的预测概率均值（pred_mean）
            label_col: ['mean', 'count'] # 每个ad的真实标签均值（true_mean）和样本数（count）
        }).dropna()  # 去除无样本的ad（箱子）
        
        # 若没有有效ad（箱子为空），返回空统计量和异常标识
        if bin_stats.empty:
        ...
        # 计算每个ad箱子的权重（该ad样本数占总样本数的比例）
        total_samples = len(df)
        bin_stats['weight'] = bin_stats['count'] / total_samples
        
        # 计算每个ad箱子的预测与真实的绝对差
        bin_stats['abs_diff'] = np.abs(bin_stats['pred_mean'] - bin_stats['true_mean'])
        
        # 计算整体ECE（加权平均绝对差）
        overall_ece = (bin_stats['weight'] * bin_stats['abs_diff']).sum()
        
        return overall_ece

    def _calculate_auc(self, df, pred_col, calib_col, label_col):
        """
        计算校准前后的AUC值（ROC曲线下面积）
        
        参数:
            df: 包含预测概率、校准后概率和真实标签的DataFrame
            pred_col: 校准前的预测概率列名
            calib_col: 校准后的预测概率列名
            label_col: 真实标签列名（二分类，通常为0/1）
            group_key: 预留分组维度参数（如按广告组计算）
            data_type: 预留数据类型参数（如训练/测试集）
        
        返回:
            tuple: (校准前AUC, 校准后AUC, AUC提升量)
        """
        # 提取真实标签（确保为二分类标签）
        ...
        # 提取校准后的预测概率
        y_calib = df[calib_col].values
        
        # 处理极端情况（标签全为一类时AUC无意义，返回-1）
        if len(np.unique(y_true)) < 2:
            return -1.0, -1.0, 0.0
        
        try:
            # 计算校准前的AUC
            auc_pre = roc_auc_score(y_true, y_pred)
            # 计算校准后的AUC
            auc_post = roc_auc_score(y_true, y_calib)
        except ValueError:
            # 处理预测值异常（如全部相同）导致的计算失败
            return -1.0, -1.0, 0.0
        
        # 计算AUC提升量（校准后 - 校准前）
        auc_lift = auc_post - auc_pre
        
        return auc_pre, auc_post, auc_lift

    def _analyze_dim_bias_distribution(self, df, dim_col, bias_type='pred', bins=None, labels=None):
        """
        按指定维度分析偏差在不同区间的分布（修改版：基于组内均值计算偏差）
        
        参数:
            df: 包含预测概率、标签、校准后概率、花费及维度列的DataFrame
            dim_col: 分析维度列名（如'ad_id'、'ad_group_id'、'app_id'）
            bias_type: 偏差类型，'pred'表示原始预测偏差，'calibrated'表示校准后偏差
            bins: 偏差区间边界，默认按0-2.0每0.1一个区间，超过2.0归为[2.0, ∞)
            labels: 区间标签，与bins对应，默认自动生成
        
        返回:
            DataFrame: 按区间排序的分布统计结果，包含区间、维度值数量、spent总和及占比
        """
        # 复制数据避免修改原表
        df = df.copy()
        
        # 1. 按维度分组，计算组内prob_col均值和label均值（核心修改点）
        # 确定使用原始预测概率还是校准后概率
        prob_col = 'pred_prob' if bias_type == 'pred' else 'calibrated_prob'
        
        # 分组计算：每个维度值的prob均值、label均值、总花费（用于后续分布统计）
        dim_grouped = df.groupby(dim_col).agg(
            prob_mean=(prob_col, 'mean'),    # 组内预测概率均值
            label_mean=('label', 'mean'),    # 组内标签均值
            total_spent=('spent', 'sum')     # 组内总花费
        ).reset_index()
        
        # 2. 计算组级偏差（偏差 = 组内prob均值 / 组内label均值，处理label均值为0的情况）
        dim_grouped['bias'] = np.where(
            dim_grouped['label_mean'] > 1e-9,  # 避免除零
            dim_grouped['prob_mean'] / dim_grouped['label_mean'],
            np.nan  # label均值接近0时偏差设为无效
        )
        
        # 3. 定义默认偏差区间（与原逻辑一致）
        if bins is None:
            bins = list(np.arange(0.0, 1.3, 0.1)) + [float('inf')]
        if labels is None:
            labels = [f'[{i:.1f},{i+0.1:.1f})' for i in np.arange(0.0, 1.2, 0.1)] + ['[1.2,∞)']
        
        # 4. 计算分布统计（基于组级数据）
        total_spent = dim_grouped['total_spent'].sum()  # 总花费（所有维度值的花费总和）
        # 调用辅助方法时传入组级数据，而非原始数据
        distribution = self._calculate_dim_distribution(
            dim_grouped, 'bias', dim_col, 'total_spent', bins, labels, total_spent
        )
        
        return distribution

    def _calculate_dim_distribution(self, df, bias_col, dim_col, value_col, bins, labels, total_value):
        """
        辅助方法：计算指定维度在各偏差区间的分布（兼容组级数据）
        """
        # 过滤无效偏差值（与原逻辑一致）
        valid_df = df.dropna(subset=[bias_col])
        
        # 初始化统计字典：key为区间标签，value包含维度值数量、价值总和、价值占比
        dist_dict = defaultdict(lambda: {
            f'{dim_col}_count': 0,    # 该区间内的维度值数量（去重，此处每个维度值唯一）
            f'{value_col}_sum': 0.0,  # 该区间内的价值总和（组内总花费之和）
            f'{value_col}_percent': 0.0  # 价值占比（%）
        })
        
        # 按区间分组处理（核心：基于组级数据统计）
        for i, (lower, upper) in enumerate(zip(bins[:-1], bins[1:])):
            # 筛选当前区间的偏差数据（组级偏差）
            mask = (valid_df[bias_col] >= lower) & (valid_df[bias_col] < upper)
            interval_df = valid_df[mask]
            
            if interval_df.empty:
                continue
            
            # 统计该区间内的维度值数量（因已按dim_col分组，直接计数即可）
            unique_dims = len(interval_df)  # 每个行代表一个唯一dim_col值
            # 统计该区间内的价值总和（组级总花费累加）
            value_sum = interval_df[value_col].sum()
            
            # 更新字典
            dist_dict[labels[i]][f'{dim_col}_count'] = unique_dims
            dist_dict[labels[i]][f'{value_col}_sum'] = value_sum
        
        # 计算每个区间的价值占比（与原逻辑一致）
        for label in dist_dict:
            if total_value > 1e-9:  # 避免除零
                dist_dict[label][f'{value_col}_percent'] = (dist_dict[label][f'{value_col}_sum'] / total_value) * 100
            else:
                dist_dict[label][f'{value_col}_percent'] = 0.0
        
        # 转换为DataFrame并按区间顺序排序（与原逻辑一致）
        dist_df = pd.DataFrame.from_dict(dist_dict, orient='index').reset_index()
        dist_df.columns = ['bias_interval', f'{dim_col}_count', f'{value_col}_sum', f'{value_col}_percentage(%)']
        dist_df['sort_key'] = dist_df['bias_interval'].apply(lambda x: labels.index(x))
        dist_df = dist_df.sort_values('sort_key').drop(columns='sort_key').reset_index(drop=True)
    ...
    def _generate_adId_online_dict(self, train_df, adid, best_dim):
        """
        为每个adid生成在线使用的词典
        在线词典格式：0/1/2^Af/s__1/2/3/4_adId/groupId/appId^A箱子边界1，箱子边界2，箱子边界3，箱子边界4；纠偏值1，纠偏值2，纠偏值3
        第一列0/1/2表示cpd、cpc、adnet场景，^A是分隔符，
        第二列f/s表示是单机游戏或者快游戏的词典，1/2/3表示纠偏维度是adId/groupId/appId的哪一类，^A是分隔符，
    ...
    def select_best_dimension_for_adid(self, adid, train_df, val_df, candidate_dims):
        ...
        for dim in candidate_dims:
            ...
            # 2. 在验证集上使用该维度进行校准并评估ctr-diff提升
            val_ctr_diff_improve, _, _ = self._calculate_ctr_diff(temp_val, 'pred_prob', 'calibrated_prob', 'label')
            if val_ctr_diff_improve < self.ctr_diff_threshold:
                reason = f"验证集CTR差异提升不足({dim})"
                reasons.append(reason)
                print(f"adid={adid} 维度={dim} 验证集CTR差异提升不足({val_ctr_diff_improve:.4f} < {self.ctr_diff_threshold})")
                continue
            
            # 如果所有条件都满足，选择该维度
            print(f"adid={adid} 选择最佳校准维度: {dim} (训练集CTR提升={ctr_diff_improve:.4f}, 验证集CTR提升={val_ctr_diff_improve:.4f}, 验证集ECE提升={ece_improve:.4f})")
            return dim, reasons
    ...
    def evaluate(self, df, evaluate_dim=None, is_train_data=False, is_val_data=False, is_test_data=False):
        ...
        eval_results_dict = self._evaluate_overall_data(df, data_type)
        results["overall"] = (eval_results_dict)
    ...
    def _evaluate_overall_data(self, df, data_type):
        """评估数据集整体的校准效果"""
        eval_results_dict = {}
        group_data = df

        if "label" not in group_data.columns:
            print(f"{data_type} 警告：数据集整体没有label列，跳过评估")
            return pd.DataFrame([]), {}
        ...
        try:
            ...
            # 2. 计算校准前后的ctr_diff
            ctr_diff_improvement, pre_ctr_diff, post_ctr_diff = self._calculate_ctr_diff(group_data, "pred_prob", "calibrated_prob", "label", "", data_type)
                
            # 3. 计算ECE, ece-filed:"ad_id", "ad_group_id", "app_id"
            ece_all = self._calculate_ece(group_data, "pred_prob", "label")
            ece_all_calibrated = self._calculate_ece(group_data, "calibrated_prob", "label")
            ece_all_improvement = ece_all - ece_all_calibrated

            filed_ece_adId = self._calculate_field_ece(group_data, "pred_prob", "label", "ad_id")
            filed_ece_adId_calibrated = self._calculate_field_ece(group_data, "calibrated_prob", "label", "ad_id")
            filed_ece_adId_improvement = filed_ece_adId - filed_ece_adId_calibrated

            filed_ece_groupId = self._calculate_field_ece(group_data, "pred_prob", "label", "ad_group_id")
            filed_ece_groupId_calibrated = self._calculate_field_ece(group_data, "calibrated_prob", "label", "ad_group_id")
            filed_ece_groupId_improvement = filed_ece_groupId - filed_ece_groupId_calibrated

            filed_ece_appId = self._calculate_field_ece(group_data, "pred_prob", "label", "app_id")
            filed_ece_appId_calibrated = self._calculate_field_ece(group_data, "calibrated_prob", "label", "app_id")
            filed_ece_appId_improvement = filed_ece_appId - filed_ece_appId_calibrated

            #4. 计算auc
            auc_pre, auc_post, auc_lift = self._calculate_auc(group_data, "pred_prob", "calibrated_prob", "label")
            ...
            num_result = {
                ...
                "ece_all": round(ece_all, 6),
                "ece_all_calibrated": round(ece_all_calibrated, 6),
                "ece_all_improvement": round(ece_all_improvement, 6),

                "filed_ece_adId": round(filed_ece_adId, 6),
                "filed_ece_adId_calibrated": round(filed_ece_adId_calibrated, 6),
                "filed_ece_adId_improvement": round(filed_ece_adId_improvement, 6),

                "filed_ece_groupId": round(filed_ece_groupId, 6),
                "filed_ece_groupId_calibrated": round(filed_ece_groupId_calibrated, 6),
                "filed_ece_groupId_improvement": round(filed_ece_groupId_improvement, 6),

                "filed_ece_appId": round(filed_ece_appId, 6),
                "filed_ece_appId_calibrated": round(filed_ece_appId_calibrated, 6),
                "filed_ece_appId_improvement": round(filed_ece_appId_improvement, 6),

                "auc_pre": round(auc_pre, 6),
                "auc_post": round(auc_post, 6),
                "auc_lift": round(auc_lift, 6),
            }

            eval_results_dict["num_result"] = num_result

            #校准前后偏差分布,"ad_id", "ad_group_id", "app_id"

            pred_adId_bias_distrib = self._analyze_dim_bias_distribution(group_data, dim_col="ad_id", bias_type='pred')
            calib_adId_bias_distrib = self._analyze_dim_bias_distribution(group_data, dim_col="ad_id", bias_type='calibrated')

            pred_groupId_bias_distrib = self._analyze_dim_bias_distribution(group_data, dim_col="ad_group_id", bias_type='pred')
            calib_groupId_bias_distrib = self._analyze_dim_bias_distribution(group_data, dim_col="ad_group_id", bias_type='calibrated')

            pred_app_id_bias_distrib = self._analyze_dim_bias_distribution(group_data, dim_col="app_id", bias_type='pred')
            calib_app_id_bias_distrib = self._analyze_dim_bias_distribution(group_data, dim_col="app_id", bias_type='calibrated')

            distribution_result = {
                "pred_adId_bias_distrib": pred_adId_bias_distrib,
                "calib_adId_bias_distrib": calib_adId_bias_distrib,

                "pred_groupId_bias_distrib": pred_groupId_bias_distrib,
                "calib_groupId_bias_distrib": calib_groupId_bias_distrib,

                "pred_app_id_bias_distrib": pred_app_id_bias_distrib,
                "calib_app_id_bias_distrib": calib_app_id_bias_distrib,
            }
            eval_results_dict["distribution_result"] = distribution_result



        except Exception as e:
            print(f"{data_type} 的 总体评估失败：{str(e)}, 跳过该数据集")
            return {}
    ...
    def _evaluate_single_dim(self, df, dim, data_type):
        ...
        if dim == "overall":
            ...
            groups = df.groupby(fields[0], observed=True) if len(fields) == 1 else df.groupby(fields, observed=True) 
            # 转换为列表以便处理
            groups = [(k, v) for k, v in groups] #每个元素是 (分组键k, 子数据集v)
...
if __name__ == "__main__":
    ...
    # 1. 解析命令行参数
    parser = argparse.ArgumentParser(description="PCVR校准流程参数（支持多日训练数据）")
    # 必传参数
    parser.add_argument("--test_day", type=str, help="测试数据日期（格式：MMDD，如1015表示10月15日）")
    parser.add_argument("--train_days", type=int, help="训练样本天数（如7表示使用7天数据）")
    parser.add_argument("--calib_method", type=int, help="校准算法（默认1：histogram_binning，2:isotonic_regression）")
    ...
    try:
        # 解析测试日期（MMDD格式）
        test_date = datetime.strptime(args.test_day, "%m%d")
        # 训练数据结束日期 = 测试日期的前一天（避免数据泄露）
        train_end_date = test_date - timedelta(days=1)
        # 训练数据开始日期 = 结束日期 - (训练天数 - 1)
        train_start_date = train_end_date - timedelta(days=args.train_days - 1)
        
        # 转换为MMDD格式字符串
        train_start_day = train_start_date.strftime("%m%d")
        train_end_day = train_end_date.strftime("%m%d")
        test_day = args.test_day
    except ValueError:
        raise ValueError(f"无效的日期格式：{args.test_day}，请使用MMDD格式（如1015）")
    
    # 3. 打印参数确认
    print("===== 运行参数 =====")
    print(f"训练数据日期范围：{train_start_day} 至 {train_end_day}（共{args.train_days}天）")
    print(f"测试数据日期：{test_day}")
    print(f"校准算法：{args.calib_method}")
    print("====================")


    # 1. 配置参数（可根据实际需求调整）
    TRAIN_DATA_PATH = f"./data/tmp_fast_pcvr_offline_Calibration_input_v1_data_{train_start_day}_{train_end_day}.txt" #!!!
    TEST_DATA_PATH = f"./data/tmp_fast_pcvr_offline_Calibration_test_v1_data_{test_day}.txt" #!!!
    # 校准算法选择："histogram_binning"（直方图分箱）或 "isotonic_regression"（保序回归）
    CALIBRATION_METHOD = "histogram_binning" # 在这里配置选择的校准算法, #!!!
    if args.calib_method == 1:
        CALIBRATION_METHOD = "histogram_binning"  
    elif args.calib_method == 2:
        CALIBRATION_METHOD = "isotonic_regression" 
    OUTPUT_TRAIN = f"./result/{CALIBRATION_METHOD}_train_calibrated_{test_day}.txt"
    OUTPUT_VAL = f"./result/{CALIBRATION_METHOD}_val_calibrated_{test_day}.txt"
    OUTPUT_TEST = f"./result/{CALIBRATION_METHOD}_test_calibrated_{test_day}.txt"
    BASE_BIN_INFO_FILE = f"./result/{CALIBRATION_METHOD}_bin_details_{test_day}.txt"  # 分箱/模型信息文件的基础名称
    ADID_BEST_DIM_FILE = f"./result/{CALIBRATION_METHOD}_adid_best_dim_{test_day}.txt"  # adid最佳维度映射文件
    ONLINE_DICT_FILE = f"./result/{CALIBRATION_METHOD}_online_dict_{test_day}.txt"  # 生成在线使用的离线词典
    ...
    # 新增配置：评估阈值
    CTR_DIFF_THRESHOLD = 0.005  # CTR差异提升阈值 #!!!
    ECE_IMPROVEMENT_THRESHOLD = 0.001  # ECE提升阈值 #!!!
    MIN_POSITIVE_SAMPLES = 1  # 最小正样本数阈值 #!!!
    MIN_NEGATIVE_SAMPLES = 100  # 最小负样本数阈值 #!!!
    ...
    DIM_MIN_SAMPLES = {
        "ad_id": 1000, #!!!100，1000
        "ad_group_id": 1000, #!!!100，1000
        "app_id": 1000, #!!!100，1000
    }
    # 校准维度优先级配置（用于维度选择）
    CALIBRATE_DIM_PRIORITY = ["ad_id", "ad_group_id", "app_id"]
    # 评估维度配置
    EVALUATE_DIMS = ["ad_id", "app_id"] #"ad_id", "ad_group_id", "app_id"
    ...
    train_df, val_df = train_test_split(
        ...
        stratify=all_train_data['label']  # 按标签列分层抽样
    )
    ...
    # 9. 输出训练数据评估结果
    print("\n=== 训练数据整体评估结果 ===")
    train_all_eval_results_dict = train_eval_results["overall"]
    for key, value in train_all_eval_results_dict["num_result"].items():
        print(f"{key}: {value}")
    for key, value in train_all_eval_results_dict["distribution_result"].items():
        print(f"{key}: \n {value}")
    ...
    # 10. 输出验证数据评估结果
    print("\n=== 验证数据整体评估结果 ===")
    val_all_eval_results_dict = val_eval_results["overall"]
    for key, value in val_all_eval_results_dict["num_result"].items():
        print(f"{key}: {value}")
    for key, value in val_all_eval_results_dict["distribution_result"].items():
        print(f"{key}: \n {value}")
    ...
    # 11. 输出测试数据评估结果
    print("\n=== 测试数据整体评估结果 ===")
    test_all_eval_results_dict = test_eval_results["overall"]
    for key, value in test_all_eval_results_dict["num_result"].items():
        print(f"{key}: {value}")
    for key, value in test_all_eval_results_dict["distribution_result"].items():
        print(f"{key}: \n {value}")

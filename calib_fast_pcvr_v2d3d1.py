...
if __name__ == "__main__":
    ...
    # 1. 配置参数（可根据实际需求调整）
    TRAIN_DATA_PATH = "./data/tmp_fast_pcvr_offline_Calibration_input_v1_data_1014.txt" #!!!
    TEST_DATA_PATH = "./data/tmp_fast_pcvr_offline_Calibration_test_v1_data_1015.txt" #!!!
    # 校准算法选择："histogram_binning"（直方图分箱）或 "isotonic_regression"（保序回归）
    CALIBRATION_METHOD = "histogram_binning"  # 在这里配置选择的校准算法, #!!!
    OUTPUT_TRAIN = f"./result/{CALIBRATION_METHOD}_train_calibrated.txt"
    OUTPUT_VAL = f"./result/{CALIBRATION_METHOD}_val_calibrated.txt"
    OUTPUT_TEST = f"./result/{CALIBRATION_METHOD}_test_calibrated.txt"
    BASE_BIN_INFO_FILE = f"./result/{CALIBRATION_METHOD}_bin_details"  # 分箱/模型信息文件的基础名称
    ADID_BEST_DIM_FILE = f"./result/{CALIBRATION_METHOD}_adid_best_dim.txt"  # adid最佳维度映射文件
    ONLINE_DICT_FILE = f"./result/{CALIBRATION_METHOD}_online_dict.txt"  # 生成在线使用的离线词典

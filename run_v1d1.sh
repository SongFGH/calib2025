...
calib_method=$3      # 第三个参数：校准算法编号（如1）

# 定义日志文件路径（包含test_day变量，确保文件名与测试日期关联）
log_file="./result/hb_calibrationV2d3_result_fast_pcvr_flow1_${test_day}.txt"

# 创建result目录（如果不存在）
mkdir -p ./result

# 执行Python脚本，并将输出重定向到日志文件
echo "开始执行校准脚本，参数：test_day=${test_day}, train_days=${train_days}, calib_method=${calib_method}"

...
def calibrate_with_fallback(df, fallback_config, n_bins=100, min_samples=100, modelname='pava', adaptive_threshold=True, use_adaptive_mixing=False):
        ...
        for dimension in dimensions:
            ...
            if conversion_rate is not None:
                ...
                if conversion_rate < 0.005:
                    current_min_samples = min_samples * 2
                elif 0.005 <= conversion_rate < 0.01:
                    current_min_samples = int(min_samples * 1.5)
    ...
    for dimension in all_dimensions:
        ...
        for key in dimension_keys:
            ...
            if len(key_df) <= 100: #!!!,限定了100，会过滤掉很多
            ...
            if modelname == 'bbq':
                ...
                # 3. 执行PAVA算法
                post_cvr = pava_algorithm(pcvr_seq, cvr_seq, weights) #!!!,保证序

                # 4. 样本压缩
                compressed_pcvr, compressed_cvr = compress_samples(pcvr_seq, post_cvr) #!!!,将相同后验值的区间进行合并
...
def get_optimal_bins(data_size, modelname='pava'):
    ...
    if modelname == 'pava':
        ...
        return max(10, min(200, data_size // 500)) #设置上下限[10,200]
    else:  # BBQ模型
        # BBQ模型的分桶策略：略少于PAVA，因为有先验信息
        return max(10, min(100, data_size // 800)) #设置上下限[10,100]
...
def main(file_path, config_path, test_path, n_bins=None, min_samples=100, ece_bins=10, modelname='pava', model_file='calibration_model.pkl', adaptive_threshold=True, threshold_history_file='threshold_history.json', use_adaptive_mixing=False):
    ...
    # 保存结果
    result_df.to_csv(file_path + modelname + '_calibrated_res_train.csv', index=False) #!!!,训练之后对自身校准后的校准值的结果保存路径
    ...
    # 保存评估指标
    with open(file_path + modelname + '_calibration_metrics_train.json', 'w') as f: #!!!,训练之后对自身校准的metric保存路径
...
def pred(test_df, calibration_funcs, fallback_decisions, ece_bins, test_path, modelname='pava'):
    ...
    print("\n校准结果和评估指标已保存")
    # 保存评估指标
    with open(test_path+ '_test_calibration_metrics.json', 'w') as f:
        json.dump(metrics, f, indent=4, default=str)
    ...
    # 保存业务评估指标
    with open(test_path + modelname + '_business_metrics.json', 'w') as f: #!!!,test的偏差相关的指标保存路径
        json.dump(business_metrics, f, indent=4, default=str)
    print(f"\n业务评估指标已保存到 {test_path}_business_metrics.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--modelname', type=str, default='pava', choices=['pava', 'bbq'], help='模型类型')

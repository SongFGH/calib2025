...
def load_data(file_path):
    ...
    if len(df.columns) == 11:  # 确认列数，包含day列和rpkid列
    ...
    df['转化label'] = df['转化label'].astype(int)
...
def create_interpolation_function(pcvr, post_cvr):
    ...
    if pcvr[-1] < 1:
        pcvr = np.concatenate((pcvr, [1]))
        post_cvr = np.concatenate((post_cvr, [1]))
...
def calibrate_with_fallback(df, fallback_config, n_bins=100, min_samples=100, modelname='pava', adaptive_threshold=True, use_adaptive_mixing=False):
    ...
    for adid in unique_adids:
        ...
        if fallback_decisions[adid]['final_dimension'] is None and len(dimensions) > 0:
            ...
            #     print(f"警告: adid {adid} 的所有维度样本数量都不足够置信，使用样本量最大的维度 {max_dimension} (样本量: {max_size})")
            # else:
            #     # 否则使用最后一个维度作为备选
    ...
    for dimension in all_dimensions:
        ...
        for key in dimension_keys:
            ...
            if modelname == 'bbq':
                ...
                # 创建并训练Platt校准器
                calibrator = PlattCalibrator(verbose=False)
...
def save_calibration_model(calibration_funcs, calibration_data, fallback_decisions, file_path):
    ...
    with open(file_path+".txt", 'w', encoding='utf-8') as out_f:
        ...
        for (dimension, key), data in calibration_data.items():
            ...
            if comp_pcvr[-1] < 1:
                comp_pcvr = np.concatenate((comp_pcvr, [1]))
                comp_cvr = np.concatenate((comp_cvr, [1]))
            ...
            pcvr_str = ','.join([f"{val:.5f}" for val in comp_pcvr])

            # 构建cvr边界字符串（逗号分隔，保留所有值）
            cvr_str = ','.join([f"{val:.5f}" for val in comp_cvr])
...
def main(file_path, config_path, test_path, n_bins=None, min_samples=100, ece_bins=10, modelname='pava', model_file='calibration_model.pkl', adaptive_threshold=True, threshold_history_file='threshold_history.json', use_adaptive_mixing=False):
    ...
    # 保存结果
    result_df.to_csv(file_path + modelname + '_train_calibrated_res.csv', index=False)
    save_calibration_model(calibration_funcs, calibration_data, fallback_decisions, model_file)

    # 保存评估指标
    with open(file_path + modelname + '_train_calibration_metrics.json', 'w') as f:
...
def pred(test_df, calibration_funcs, fallback_decisions, ece_bins, test_path, modelname='pava'):
    ...
    # 保存结果
    result_df.to_csv(test_path+ modelname+ '_test_calibrated_results_with_fallback.csv', index=False)
    ...
    # 保存业务评估指标
    with open(test_path + modelname + '_test_business_metrics.json', 'w') as f:
        json.dump(business_metrics, f, indent=4, default=str)

...
class PlattCalibrator:
    ...
    def fit(self, scores, targets):
        ...
        if self.verbose:
            ...
            loss = -np.sum(targets * np.log(calibrated_probs + 1e-10) + (1 - targets) * np.log(1 - calibrated_probs + 1e-10))
...
def example_usage():
    ...
    nums_normal = np.random.normal(0.5, 100, n_samples)
    targets = (nums_normal - nums_normal.min()) / (nums_normal.max() - nums_normal.min())
